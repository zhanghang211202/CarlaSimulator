...
class SensorManager:
    ...
    def init_sensor(self, sensor_type, transform, attached, sensor_options):
        if sensor_type == 'RGBCamera':
            ...
            camera_bp.set_attribute('sensor_tick', '2')
            for key in sensor_options:
                camera_bp.set_attribute(key, sensor_options[key])

            camera = self.world.spawn_actor(camera_bp, transform, attach_to=attached)
            camera.listen(lambda data: self.sensor_callback(data, sensor_type))
    ...
    def sensor_callback(self,image,sensor_type):
        ...
        if(sensor_type=='RGBCamera'):
            ...
            img.save('carla_images/rgb/town06/100/%08d.png' % (image.frame))
        elif(sensor_type=='SEGCamera'):
            ...
            if self.display_man.render_enabled():
                self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
            img = Image.fromarray(temp, mode="RGB")
            img.save('carla_images/seg/town06/100/%08d.png' % (image.frame))
    ...
    #         self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #
    #     image.save_to_disk('carla_images/rgb/%08d' % (image.frame))
    ...
    #         self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
    #
    #     image.save_to_disk('carla_images/seg/%08d' % (image.frame))
    #     t_end = self.timer.time()
    #     self.time_processing += (t_end - t_start)
    #     self.tics_processing += 1
...
def run_simulation(args, client):
    ...
    try:
        ...
        for actor in actor_list:
            if actor.attributes.get('role_name') == vehicle_name:
                found_vehicle = actor
                break

        vehicle_list.append(found_vehicle)
        # found_vehicle.set_autopilot(True)
        ...
        rgb_sensor = SensorManager(world, display_manager, 'RGBCamera', carla.Transform(carla.Location(x=1.5, z=2.4), carla.Rotation(yaw=+00)),
                      found_vehicle, {}, display_pos=[0, 0])
        seg_sensor = SensorManager(world, display_manager, 'SEGCamera', carla.Transform(carla.Location(x=1.5, z=2.4), carla.Rotation(yaw=+00)),
                      found_vehicle, {}, display_pos=[0, 1])
        # SensorManager(world, display_manager, 'RGBCamera', carla.Transform(carla.Location(x=0, z=2.4), carla.Rotation(yaw=+90)),
...
def main():
    ...
    argparser.add_argument(
        ...
        default='7680x2160',
        help='window resolution (default: 2048x512)')
