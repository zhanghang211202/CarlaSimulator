...
town12_3_2_z_list = [36626.0/100-0.5]
# modify here
angle_list = [11,30,90]
...
def process_rgb_image(image):
    ...
    print('before save')
    img_name = 'carla_images/images/{}/{}/{:0>8}.png'.format(town_name, distance_name, image.frame)
    img.save(img_name)
...
def process_seg_image(img_frame,image):
    ...
    img_name = 'carla_images/labels/{}/{}/{:0>8}.png'.format(town_name, distance_name, img_frame.frame)
    img.save(img_name)
...
for index, spawn_location in enumerate(loc_list):
    ...
    for k in angle_list:
        print('new angle')
        for j in range(0, 4):
            weather.sun_azimuth_angle = 90 * j
            weather.sun_altitude_angle = k
