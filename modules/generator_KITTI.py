...
def lidar_carllback(data, lidar_queue):
...
class Sensor:
    ...
    def __init__(self, vehicle, world, actor_list, folder_output, transform):
        ...
        self.map_name = world.get_map().name.split('/')[-1]
...
class Camera(Sensor):
    ...
    def save(self, color_converter=carla.ColorConverter.Raw):
        ...
        if not self.queue.empty():
            ...
            file_path = self.frame_output+"/%04d_%d_%s.png" %(self.sensor_frame_id, self.sensor_id, self.map_name)
...
class HDL64E(Sensor):
    ...
    def __init__(self, vehicle, world, actor_list, folder_output, transform):
        ...
        self.sensor = world.spawn_actor(self.bp, transform, attach_to=vehicle)
        actor_list.append(self.sensor)
        self.sensor.listen(lambda data: lidar_carllback((-data.timestamp, data), self.queue))
        self.sensor_id = self.__class__.sensor_id_glob;
        self.__class__.sensor_id_glob += 1
        self.folder_output = folder_output
        ...
        if self.bp.get_attribute('rotation_frequency') is not None:
            ...
            self.packet_per_frame = 1/(self.bp.get_attribute(10*settings.fixed_delta_seconds))
    ...
    def set_attributes(self, blueprint_library):
        ...
        lidar_bp.set_attribute('range', '100.0')    # 80.0 m
