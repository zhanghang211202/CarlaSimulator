...
class CameraManager(object):
    def __init__(self, parent_actor, hud, gamma_correction):
        ...
        if not self._parent.type_id.startswith("walker.pedestrian"):
            ...
            self._camera_transforms = [
                # (carla.Transform(carla.Location(x=0.30, y=0, z=1.70), carla.Rotation(pitch=0, yaw=0, roll=0)), Attachment.SpringArm),
                (carla.Transform(carla.Location(x=0.30, y=0, z=1.70), carla.Rotation(pitch=0, yaw=0, roll=0))),
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            self.surface = pygame.surfarray.make_surface(array.swapaxes(0, 1))
        if self.recording:
            image.save_to_disk('carla_images/%08d' % (image.frame))
